...
const Sidebar = () => {
  ...
  return (
    <div className="w-[250px] h-full bg-slate-100  ">
      <div className="flex justify-between px-2 py-2 items-center MainTextColor">
        ...
        <RxHamburgerMenu className="text-[20px] hover:text-gray-500 cursor-pointer " />
      </div>
      ...
      <div className="">
        {SideBaritems.map((sidebar, index) => (
          <Link
            to={sidebar.to}
            key={index}
            className="flex  items-center gap-2 text-[18px] uppercase p-3 mt-1 font-semibold hover:bg-[#25AE9C] hover:rounded-l-md hover:text-white cursor-pointer  duration-300 hover:scale-105 ml-3"
          >
            {sidebar.icons}
            <label className="cursor-pointer">{sidebar.item}</label>
          </Link>
        ))}
      </div>
    </div>
